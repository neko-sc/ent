...
func (_q *GroupQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("testschema: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (_q *GroupQuery) prepareQuery(ctx context.Context) error {
	for _, f := range _q.ctx.Fields {
		if !group.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("testschema: invalid field %q for query", f)}
		}
	}
	if _q.path != nil {
