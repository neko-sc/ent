...
func (b *UserCreate) check() error {
	...
	switch b.driver.Dialect() {
	case dialect.Postgres, dialect.SQLite:
		if _, present := b.present[user.FieldEmail]; b.fromBuilder && !present {
	...
	switch b.driver.Dialect() {
	case dialect.Postgres, dialect.SQLite:
		if _, present := b.present[user.FieldRole]; b.fromBuilder && !present {
			return &ValidationError{Name: "role", err: errors.New(`ent: missing required field "User.role"`)}
		}
	}

	if b.mutation.insert.expressions[user.FieldRole] == nil {
		v := b.mutation.insert.Role
	...
	switch b.driver.Dialect() {
	case dialect.Postgres, dialect.SQLite:
		if _, present := b.present[user.FieldTags]; b.fromBuilder && !present {
	...
	switch b.driver.Dialect() {
	case dialect.Postgres, dialect.SQLite:
		if _, present := b.present[user.FieldProfile]; b.fromBuilder && !present {
...
func (_c *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec, error) {
	...
	if _, present := _c.present[user.FieldEmail]; !_c.fromBuilder || present {
	...
	if _, present := _c.present[user.FieldRole]; !_c.fromBuilder || present {
		value := _c.mutation.insert.Role
		_spec.SetField(user.FieldRole, field.TypeEnum, value)
	}

	if _, present := _c.present[user.FieldTags]; !_c.fromBuilder || present {
		value := _c.mutation.insert.Tags
		_spec.SetField(user.FieldTags, field.TypeArray, value)
	}

	if _, present := _c.present[user.FieldProfile]; !_c.fromBuilder || present {
		value := _c.mutation.insert.Profile
	...
	if value, ok := _c.mutation.insert.Avatar.Get(); ok {
		_spec.SetField(user.FieldAvatar, field.TypeBytes, value)
	}
	if _c.mutation.insert.Avatar.IsNull() {
		_spec.SetField(user.FieldAvatar, field.TypeBytes, nil)
	}
