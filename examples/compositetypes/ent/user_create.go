...
func (b *UserCreate) check() error {
	...
	case dialect.Postgres, dialect.SQLite:
		if _, present := b.present[user.FieldAddress]; b.fromBuilder && !present {
...
func (_c *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec, error) {
	...
	if _, present := _c.present[user.FieldAddress]; !_c.fromBuilder || present {
		value := _c.mutation.insert.Address

		converted, err := user.ValueScanner.Address.Value(value)
		if err != nil {
			return nil, nil, err
		}
		_spec.SetField(user.FieldAddress, field.TypeString, converted)
...
func (u *UserUpsert) Set[T any](column ent.ColumnOf[entity.User, T], value T) *UserUpsert {
	...
	case user.FieldAddress:
		...
		converted, err := user.ValueScanner.Address.Value(any(value).(*schema2.Address))
		if err != nil {
