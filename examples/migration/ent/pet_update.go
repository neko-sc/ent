...
func (b *PetUpdate) check() error {
	...
	if b.mutation.patch.Weight.IsNull() {
		return &ValidationError{Name: "weight", err: errors.New(`ent: field "Pet.weight" is not nullable`)}
	}

	if b.mutation.patch.BestFriendID.IsNull() {
		return &ValidationError{Name: "best_friend_id", err: errors.New(`ent: field "Pet.best_friend_id" is not nullable`)}
	}
	...
	if b.mutation.patch.BestFriendID.IsNull() {
		return &ValidationError{Name: "best_friend", err: errors.New(`ent: clearing required edge "Pet.best_friend"`)}
	}

	if b.mutation.patch.OwnerID.IsNull() {
		return &ValidationError{Name: "owner", err: errors.New(`ent: clearing required edge "Pet.owner"`)}
	}
...
func (_u *PetUpdate) sqlSave(ctx context.Context) (_node int, err error) {
	...
	if _u.mutation.patch.BestFriendID.IsNull() || _u.mutation.patch.clearedEdges[pet.EdgeBestFriend] {
	...
	if nodes := _u.mutation.patch.bestfriendIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2O,
			Inverse: false,
			Table:   pet.BestFriendTable,
...
func (b *PetUpdateOne) Columns(columns ...ent.EntityColumn[entity.Pet]) *PetUpdateOne {
	if len(columns) == 0 {
