...
func (b *UserCreate) check() error {
	...
	switch b.driver.Dialect() {
	case dialect.Postgres, dialect.SQLite:
		if _, present := b.present[user.FieldFirstName]; b.fromBuilder && !present {
	...
	switch b.driver.Dialect() {
	case dialect.Postgres, dialect.SQLite:
		if _, present := b.present[user.FieldLastName]; b.fromBuilder && !present {
...
func (_c *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec, error) {
	...
	if _, present := _c.present[user.FieldFirstName]; !_c.fromBuilder || present {
		value := _c.mutation.insert.FirstName
		_spec.SetField(user.FieldFirstName, field.TypeString, value)
	}

	if _, present := _c.present[user.FieldLastName]; !_c.fromBuilder || present {
		value := _c.mutation.insert.LastName
	...
	if _c.mutation.insert.Tags.IsNull() {
		_spec.SetField(user.FieldTags, field.TypeJSON, nil)
	}

	_spec.Expressions = _c.mutation.insert.expressions

	if nodes := _c.mutation.insert.cardsIDs(); len(nodes) > 0 {
