...
func (b *PetCreate) SetEdge[N, K any](edge ent.UniqueRelation[entity.Pet, N, K], id K) *PetCreate {
	...
	case pet.EdgeBestFriend:
		...
		b.present[pet.FieldBestFriendID] = struct{}{}
	case pet.EdgeOwner:
		if b.present == nil {
			b.present = make(map[string]struct{})
		}
...
func (b *PetCreate) defaults() error {
	...
	if b.mutation.insert.ID.IsUnset() && b.mutation.insert.expressions[pet.FieldID] == nil {
		if pet.DefaultID == nil {
			return fmt.Errorf("ent: uninitialized pet.DefaultID")
		}
		b.mutation.insert.ID = ent.Some(pet.DefaultID())
	}

	if b.mutation.insert.OwnerID.IsUnset() && b.mutation.insert.expressions[pet.FieldOwnerID] == nil {

		b.mutation.insert.OwnerID = ent.Some(pet.DefaultOwnerID)
	}
...
func (b *PetCreate) check() error {
	...
	if b.mutation.insert.ID.IsNull() {
		return &ValidationError{Name: "id", err: errors.New(`ent: field "Pet.id" is not nullable`)}
	}
	...
	switch b.driver.Dialect() {
	case dialect.Postgres, dialect.SQLite:
		if _, present := b.present[pet.FieldWeight]; b.fromBuilder && !present {
	...
	switch b.driver.Dialect() {
	case dialect.Postgres, dialect.SQLite:
		if _, present := b.present[pet.FieldBestFriendID]; b.fromBuilder && !present {
			return &ValidationError{Name: "best_friend_id", err: errors.New(`ent: missing required field "Pet.best_friend_id"`)}
		}
	}

	if b.mutation.insert.OwnerID.IsNull() {
		return &ValidationError{Name: "owner_id", err: errors.New(`ent: field "Pet.owner_id" is not nullable`)}
	}

	if len(b.mutation.insert.bestfriendIDs()) == 0 {
...
func (_c *PetCreate) createSpec() (*Pet, *sqlgraph.CreateSpec, error) {
	...
	if _, present := _c.present[pet.FieldWeight]; !_c.fromBuilder || present {
		value := _c.mutation.insert.Weight
		_spec.SetField(pet.FieldWeight, field.TypeFloat64, value)
	}

	_spec.Expressions = _c.mutation.insert.expressions

	if nodes := _c.mutation.insert.bestfriendIDs(); len(nodes) > 0 {
