...
func (_q *NodeQuery) Where(predicates ...ent.Predicate[entity.Node]) *NodeQuery {
	_q.predicates = append(_q.predicates, predicates...)
	return _q
}

func (_q *NodeQuery) WhereP(predicates ...func(*sql.Selector)) *NodeQuery {
...
func (_q *NodeQuery) Join(table string, on ...func(*sql.Selector)) *NodeQuery {
	return _q.JoinAs(table, "", on...)
}

func (_q *NodeQuery) JoinAs(table, alias string, on ...func(*sql.Selector)) *NodeQuery {
...
func (_q *NodeQuery) LeftJoin(table string, on ...func(*sql.Selector)) *NodeQuery {
	return _q.LeftJoinAs(table, "", on...)
}

func (_q *NodeQuery) LeftJoinAs(table, alias string, on ...func(*sql.Selector)) *NodeQuery {
	on = append([]func(*sql.Selector){}, on...)
	_q.joins = append(_q.joins, func(selector *sql.Selector) {
		joined := sql.Dialect(selector.Dialect()).Table(table)
		if alias != "" {
...
func (_q *NodeQuery) Order(o ...ent.OrderOption[entity.Node]) *NodeQuery {
...
func (_q *NodeQuery) IDs(ctx context.Context) (ids []int, err error) {
	...
	if ids, err = ent.Values(ctx, _q.Select(node.ID), node.ID); err != nil {
		return nil, err
	}
	return ids, nil
}
...
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
func (_q *NodeQuery) GroupBy(columns ...ent.EntityColumn[entity.Node]) *NodeGroupBy {
	if len(columns) == 0 {
		panic("ent: GroupBy requires at least one column")
	}
	return &NodeGroupBy{query: _q, columns: append([]ent.EntityColumn[entity.Node](nil), columns...)}
}
...
//		Select(node.Value).
//		Scan(ctx, &v)
func (_q *NodeQuery) Select(selections ...ent.Selection) *NodeSelect {
	return &NodeSelect{query: _q, selections: append([]ent.Selection(nil), selections...)}
}

// Aggregate returns a NodeSelect configured with the given aggregations.
func (_q *NodeQuery) Aggregate(selections ...ent.Selection) *NodeSelect {
...
func (_q *NodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Node, error) {
	...
	if query := _q.withPrev; query != nil {
	...
	if len(_q.withCounts) > 0 {
		...
		parents := make(map[int][]*Node, len(nodes))
		...
		for _, edge := range _q.withCounts {
			...
			for rows.Next() {
				values, err := (*Node)(nil).scanValues([]string{node.FieldID})
				if err != nil {
					rows.Close()
					return nil, err
				}
				...
				decoded := &Node{}
				if err := decoded.assignValues([]string{node.FieldID}, values); err != nil {
					rows.Close()
					return nil, err
				}
...
func (_q *NodeQuery) loadNext(ctx context.Context, query *NodeQuery, nodes []*Node, init func(*Node), assign func(*Node, *Node)) error {
	...
	if query.ctx.Limit != nil || query.ctx.Offset != nil {
		...
		query.modifiers = append(query.modifiers, func(selector *sql.Selector) {
			...
			partition := selector.C(node.NextColumn)

			(&sqlgraph.NeighborsLimit{RowNumber: "ent_row_number", DefaultOrderField: node.FieldID, Offset: offset}).Modifier(partition, limit)(selector)
		})
	}
...
type NodeGroupBy struct {
	query      *NodeQuery
	columns    []ent.EntityColumn[entity.Node]
	aggregates []ent.Selection
}

func (_g *NodeGroupBy) Aggregate(selections ...ent.Selection) *NodeGroupBy {
...
func (_g *NodeGroupBy) Rows(ctx context.Context) ([]*ent.Row, error) {
	return _g.selectQuery().Rows(ctx)
}

func (_g *NodeGroupBy) selectQuery() *NodeSelect {
...
type NodeSelect struct {
	...
	groups     []ent.EntityColumn[entity.Node]
}

func (_s *NodeSelect) Aggregate(selections ...ent.Selection) *NodeSelect {
	_s.selections = append(_s.selections, selections...)
	return _s
}

func (_s *NodeSelect) Row(ctx context.Context) (*ent.Row, error) {
...
func (_s *NodeSelect) sqlQuery(ctx context.Context) (*sql.Selector, error) {
	root := _s.query.Clone()
	root.ctx.Fields = nil
	for _, selection := range _s.selections {
		if column := selection.Ref(); column.Name != "" && (column.Table == "" || column.Table == node.Table) {
...
func (_s *NodeSelect) Scan(ctx context.Context, value any) error {
...
func (_s *NodeSelect) Rows(ctx context.Context) ([]*ent.Row, error) {
	if len(_s.selections) == 0 {
