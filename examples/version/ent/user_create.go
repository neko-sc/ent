...
func (b *UserCreate) defaults() error {
	...
	if b.mutation.insert.Version.IsUnset() && b.mutation.insert.expressions[user.FieldVersion] == nil {
		if user.DefaultVersion == nil {
			return fmt.Errorf("ent: uninitialized user.DefaultVersion")
		}
		b.mutation.insert.Version = ent.Some(user.DefaultVersion())
	}
...
func (b *UserCreate) check() error {
	...
	if b.mutation.insert.Version.IsNull() {
		return &ValidationError{Name: "version", err: errors.New(`ent: field "User.version" is not nullable`)}
	}

	switch b.driver.Dialect() {
	case dialect.Postgres, dialect.SQLite:
		if _, present := b.present[user.FieldStatus]; b.fromBuilder && !present {
...
func (_c *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec, error) {
	...
	if value, ok := _c.mutation.insert.Version.Get(); ok {
		_spec.SetField(user.FieldVersion, field.TypeInt64, value)
	}
	if _c.mutation.insert.Version.IsNull() {
		_spec.SetField(user.FieldVersion, field.TypeInt64, nil)
	}

	if _, present := _c.present[user.FieldStatus]; !_c.fromBuilder || present {
