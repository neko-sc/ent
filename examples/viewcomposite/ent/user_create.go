...
func (b *UserCreate) check() error {
	...
	switch b.driver.Dialect() {
	case dialect.Postgres, dialect.SQLite:
		if _, present := b.present[user.FieldPublicInfo]; b.fromBuilder && !present {
	...
	switch b.driver.Dialect() {
	case dialect.Postgres, dialect.SQLite:
		if _, present := b.present[user.FieldPrivateInfo]; b.fromBuilder && !present {
...
func (_c *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec, error) {
	...
	if _, present := _c.present[user.FieldPublicInfo]; !_c.fromBuilder || present {
		value := _c.mutation.insert.PublicInfo
		_spec.SetField(user.FieldPublicInfo, field.TypeString, value)
	}

	if _, present := _c.present[user.FieldPrivateInfo]; !_c.fromBuilder || present {
		value := _c.mutation.insert.PrivateInfo
