...
func (b *UserUpdate) check() error {
	...
	if b.mutation.patch.PublicInfo.IsNull() {
		return &ValidationError{Name: "public_info", err: errors.New(`ent: field "User.public_info" is not nullable`)}
	}

	if b.mutation.patch.PrivateInfo.IsNull() {
		return &ValidationError{Name: "private_info", err: errors.New(`ent: field "User.private_info" is not nullable`)}
	}
...
func (_u *UserUpdate) sqlSave(ctx context.Context) (_node int, err error) {
	...
	if value, ok := _u.mutation.patch.PublicInfo.Get(); ok {
		_spec.SetField(user.FieldPublicInfo, field.TypeString, value)
	}
	if value, ok := _u.mutation.patch.PrivateInfo.Get(); ok {
