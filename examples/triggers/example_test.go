...
import (
	...
	"github.com/neko-sc/ent/examples/triggers/ent"
	user "github.com/neko-sc/ent/examples/triggers/ent/user"
...
func TestTriggers(t *testing.T) {
	...
	client.User.Create().Set(user.Name, "a8m").ExecX(ctx)
	...
	client.User.Update().Set(user.Name, "Ariel").ExecX(ctx)
	logs = client.UserAuditLog.Query().Order(userauditlog.ID.Asc()).AllX(ctx)
