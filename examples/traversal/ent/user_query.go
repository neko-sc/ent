...
func (_q *UserQuery) QueryManage() *GroupQuery {
	query := (&GroupClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	cloned := &UserQuery{
		...
		withManage:  _q.withManage.Clone(),
...
func (_q *UserQuery) WithManage(opts ...func(*GroupQuery)) *UserQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withManage; query != nil {
		if err := _q.loadManage(ctx, query, nodes,
...
func (_q *UserQuery) loadManage(ctx context.Context, query *GroupQuery, nodes []*User, init func(*User), assign func(*User, *Group)) error {
	query = query.Clone()
	...
	if query.ctx.Limit != nil || query.ctx.Offset != nil {
		...
		query.modifiers = append(query.modifiers, func(selector *sql.Selector) {
			...
			partition := selector.C(user.ManageColumn)
	...
	query.withFKs = true
	query.Where(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ManageColumn), fks...))
	})
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.group_admin
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_admin" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
