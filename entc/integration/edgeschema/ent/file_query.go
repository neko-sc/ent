...
func (_q *FileQuery) loadProcesses(ctx context.Context, query *ProcessQuery, nodes []*File, init func(*File), assign func(*File, *Process)) error {
	...
	if query.ctx.Limit != nil || query.ctx.Offset != nil {
		...
		query.modifiers = append(query.modifiers, func(selector *sql.Selector) {
			...
			joined, _ := selector.JoinedTable(file.ProcessesTable)
			partition := joined.C(file.ProcessesPrimaryKey[1])

			(&sqlgraph.NeighborsLimit{RowNumber: "ent_row_number", DefaultOrderField: process.FieldID, Offset: offset}).Modifier(partition, limit)(selector)
		})
	}
	...
	byNeighborID := make(map[int]*Process, len(neighbors))
