...
func (_c *CustomTypeCreate) createSpec() (*CustomType, *sqlgraph.CreateSpec, error) {
	...
	if value, ok := _c.mutation.insert.Tz0.Get(); ok {
		_spec.SetField(customtype.FieldTz0, field.TypeTime, value)
	}
	if _c.mutation.insert.Tz0.IsNull() {
		_spec.SetField(customtype.FieldTz0, field.TypeTime, nil)
	}

	if value, ok := _c.mutation.insert.Tz3.Get(); ok {
		_spec.SetField(customtype.FieldTz3, field.TypeTime, value)
	}
	if _c.mutation.insert.Tz3.IsNull() {
		_spec.SetField(customtype.FieldTz3, field.TypeTime, nil)
	}
