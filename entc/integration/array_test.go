...
func Arrays(t *testing.T, client *ent.Client) {
	...
	require.Equal(t, 1, client.User.Query().Where(user.Tags.Contains("go"), user.Tags.HasAny("sql", "missing"), user.Tags.Contains("go", "sql"), user.Tags.LenGT(1), user.Scores.Contains(2)).CountX(ctx))
	...
	require.Equal(t, 1, client.User.Query().Where(user.Tags.Contains()).CountX(ctx))
	require.Zero(t, client.User.Query().Where(user.Tags.HasAny()).CountX(ctx))
	created.Update().Clear(user.Tags).SaveX(ctx)
	require.Nil(t, client.User.GetX(ctx, created.ID).Tags)
	require.Zero(t, client.User.Query().Where(user.Tags.Contains()).CountX(ctx))
	require.Zero(t, client.User.Query().Where(user.Not(user.Tags.HasAny())).CountX(ctx))
