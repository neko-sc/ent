...
import (
	...
	"github.com/neko-sc/ent/entc/integration/multischema/ent/migrate"
...
func (c *ParentClient) Create() *ParentCreate {
	...
	return &ParentCreate{config: c.config, mutation: mutation, fromBuilder: true}
}

func (c *ParentClient) Insert(value ParentInsert) *ParentCreate {
	mutation := newParentMutation(c.config, OpCreate)
	mutation.insert = &value
	return &ParentCreate{config: c.config, mutation: mutation}
}

func (c *ParentClient) InsertBulk(values ...ParentInsert) *ParentCreateBulk {
	builders := make([]*ParentCreate, len(values))
	for index := range values {
		builders[index] = c.Insert(values[index])
	}
	return c.CreateBulk(builders...)
...
func (c *ParentClient) Update() *ParentUpdate {
	...
	return &ParentUpdate{config: c.config, mutation: mutation}
}
...
func (c *ParentClient) UpdateOneID(id int) *ParentUpdateOne {
	mutation := newParentMutation(c.config, OpUpdateOne)
	mutation.id = &id
	return &ParentUpdateOne{config: c.config, mutation: mutation}
}
...
func (c *ParentClient) Delete() *ParentDelete {
	...
	return &ParentDelete{config: c.config, mutation: mutation}
}
...
func (c *ParentClient) DeleteOneID(id int) *ParentDeleteOne {
	builder := c.Delete().Where(parent.ID.EQ(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
...
func (c *ParentClient) Get(ctx context.Context, id int) (*Parent, error) {
	return c.Query().Where(parent.ID.EQ(id)).Only(ctx)
}
...
func (c *ParentClient) mutate(ctx context.Context, m *ParentMutation) (any, error) {
	switch m.Op() {
	case OpCreate:
		return (&ParentCreate{config: c.config, mutation: m}).Save(ctx)
	case OpUpdate:
		return (&ParentUpdate{config: c.config, mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&ParentUpdateOne{config: c.config, mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&ParentDelete{config: c.config, mutation: m}).Exec(ctx)
	default:
