...
type CardQuery struct {
	config
	ctx        *QueryContext
	order      []ent.OrderOption[entity.Card]
...
func (_q *CardQuery) Where(predicates ...ent.Predicate[entity.Card]) *CardQuery {
	_q.predicates = append(_q.predicates, predicates...)
	return _q
}

func (_q *CardQuery) WhereP(predicates ...func(*sql.Selector)) *CardQuery {
...
func (_q *CardQuery) Join(table string, on ...func(*sql.Selector)) *CardQuery {
	return _q.JoinAs(table, "", on...)
}

func (_q *CardQuery) JoinAs(table, alias string, on ...func(*sql.Selector)) *CardQuery {
...
func (_q *CardQuery) LeftJoin(table string, on ...func(*sql.Selector)) *CardQuery {
	return _q.LeftJoinAs(table, "", on...)
}

func (_q *CardQuery) LeftJoinAs(table, alias string, on ...func(*sql.Selector)) *CardQuery {
	on = append([]func(*sql.Selector){}, on...)
	_q.joins = append(_q.joins, func(selector *sql.Selector) {
		joined := sql.Dialect(selector.Dialect()).Table(table)
		if alias != "" {
...
// Card, readable with Edges.Count. Only neighbors matching all predicates are
...
func (_q *CardQuery) WithCount[N, K any](edge ent.Relation[entity.Card, N, K], predicates ...ent.Predicate[N]) *CardQuery {
...
func (_q *CardQuery) Order(o ...ent.OrderOption[entity.Card]) *CardQuery {
...
func (_q *CardQuery) IDs(ctx context.Context) (ids []int, err error) {
	...
	if ids, err = ent.Values(ctx, _q.Select(card.ID), card.ID); err != nil {
		return nil, err
	}
	return ids, nil
}
...
func (_q *CardQuery) Clone() *CardQuery {
	...
	cloned := &CardQuery{
		config:     _q.config,
		ctx:        _q.ctx.Clone(),
		order:      append([]ent.OrderOption[entity.Card]{}, _q.order...),
		predicates: append([]ent.Predicate[entity.Card]{}, _q.predicates...),
	...
	if _q.withNamedSpec != nil {
		cloned.withNamedSpec = make(map[string]*SpecQuery, len(_q.withNamedSpec))
		for name, query := range _q.withNamedSpec {
			cloned.withNamedSpec[name] = query.Clone()
		}
	}
...
//
//	client.Card.Query().
//		GroupBy(card.CreateTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
func (_q *CardQuery) GroupBy(columns ...ent.EntityColumn[entity.Card]) *CardGroupBy {
	if len(columns) == 0 {
		panic("ent: GroupBy requires at least one column")
	}
	return &CardGroupBy{query: _q, columns: append([]ent.EntityColumn[entity.Card](nil), columns...)}
}
...
//
//	client.Card.Query().
//		Select(card.CreateTime).
//		Scan(ctx, &v)
func (_q *CardQuery) Select(selections ...ent.Selection) *CardSelect {
	return &CardSelect{query: _q, selections: append([]ent.Selection(nil), selections...)}
}

// Aggregate returns a CardSelect configured with the given aggregations.
func (_q *CardQuery) Aggregate(selections ...ent.Selection) *CardSelect {
...
func (_q *CardQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Card, error) {
	...
	if len(_q.withCounts) > 0 {
		...
		parents := make(map[int][]*Card, len(nodes))
		...
		for _, edge := range _q.withCounts {
			...
			for rows.Next() {
				values, err := (*Card)(nil).scanValues([]string{card.FieldID})
				if err != nil {
					rows.Close()
					return nil, err
				}
				...
				decoded := &Card{}
				if err := decoded.assignValues([]string{card.FieldID}, values); err != nil {
					rows.Close()
					return nil, err
				}
...
func (_q *CardQuery) loadSpec(ctx context.Context, query *SpecQuery, nodes []*Card, init func(*Card), assign func(*Card, *Spec)) error {
	...
	if query.ctx.Limit != nil || query.ctx.Offset != nil {
		...
		query.modifiers = append(query.modifiers, func(selector *sql.Selector) {
			...
			joined, _ := selector.JoinedTable(card.SpecTable)
			partition := joined.C(card.SpecPrimaryKey[1])

			(&sqlgraph.NeighborsLimit{RowNumber: "ent_row_number", DefaultOrderField: spec.FieldID, Offset: offset}).Modifier(partition, limit)(selector)
		})
	}
	...
	byNeighborID := make(map[int]*Spec, len(neighbors))
...
type CardGroupBy struct {
	query      *CardQuery
	columns    []ent.EntityColumn[entity.Card]
	aggregates []ent.Selection
}

func (_g *CardGroupBy) Aggregate(selections ...ent.Selection) *CardGroupBy {
...
func (_g *CardGroupBy) Rows(ctx context.Context) ([]*ent.Row, error) {
	return _g.selectQuery().Rows(ctx)
}

func (_g *CardGroupBy) selectQuery() *CardSelect {
...
type CardSelect struct {
	...
	groups     []ent.EntityColumn[entity.Card]
}

func (_s *CardSelect) Aggregate(selections ...ent.Selection) *CardSelect {
	_s.selections = append(_s.selections, selections...)
	return _s
}

func (_s *CardSelect) Row(ctx context.Context) (*ent.Row, error) {
...
func (_s *CardSelect) sqlQuery(ctx context.Context) (*sql.Selector, error) {
	root := _s.query.Clone()
	root.ctx.Fields = nil
	for _, selection := range _s.selections {
		if column := selection.Ref(); column.Name != "" && (column.Table == "" || column.Table == card.Table) {
...
func (_s *CardSelect) Scan(ctx context.Context, value any) error {
...
func (_s *CardSelect) Rows(ctx context.Context) ([]*ent.Row, error) {
	if len(_s.selections) == 0 {
