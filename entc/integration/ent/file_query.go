...
type FileQuery struct {
	config
	ctx        *QueryContext
	order      []ent.OrderOption[entity.File]
...
func (_q *FileQuery) Where(predicates ...ent.Predicate[entity.File]) *FileQuery {
	_q.predicates = append(_q.predicates, predicates...)
	return _q
}

func (_q *FileQuery) WhereP(predicates ...func(*sql.Selector)) *FileQuery {
...
func (_q *FileQuery) Join(table string, on ...func(*sql.Selector)) *FileQuery {
	return _q.JoinAs(table, "", on...)
}

func (_q *FileQuery) JoinAs(table, alias string, on ...func(*sql.Selector)) *FileQuery {
...
func (_q *FileQuery) LeftJoin(table string, on ...func(*sql.Selector)) *FileQuery {
	return _q.LeftJoinAs(table, "", on...)
}

func (_q *FileQuery) LeftJoinAs(table, alias string, on ...func(*sql.Selector)) *FileQuery {
	on = append([]func(*sql.Selector){}, on...)
	_q.joins = append(_q.joins, func(selector *sql.Selector) {
		joined := sql.Dialect(selector.Dialect()).Table(table)
		if alias != "" {
...
// File, readable with Edges.Count. Only neighbors matching all predicates are
...
func (_q *FileQuery) WithCount[N, K any](edge ent.Relation[entity.File, N, K], predicates ...ent.Predicate[N]) *FileQuery {
...
func (_q *FileQuery) Order(o ...ent.OrderOption[entity.File]) *FileQuery {
...
func (_q *FileQuery) IDs(ctx context.Context) (ids []int, err error) {
	...
	if ids, err = ent.Values(ctx, _q.Select(file.ID), file.ID); err != nil {
		return nil, err
	}
	return ids, nil
}
...
func (_q *FileQuery) Clone() *FileQuery {
	...
	cloned := &FileQuery{
		config:     _q.config,
		ctx:        _q.ctx.Clone(),
		order:      append([]ent.OrderOption[entity.File]{}, _q.order...),
		predicates: append([]ent.Predicate[entity.File]{}, _q.predicates...),
	...
	if _q.withNamedField != nil {
		cloned.withNamedField = make(map[string]*FieldTypeQuery, len(_q.withNamedField))
		for name, query := range _q.withNamedField {
			cloned.withNamedField[name] = query.Clone()
		}
	}
...
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
func (_q *FileQuery) GroupBy(columns ...ent.EntityColumn[entity.File]) *FileGroupBy {
	if len(columns) == 0 {
		panic("ent: GroupBy requires at least one column")
	}
	return &FileGroupBy{query: _q, columns: append([]ent.EntityColumn[entity.File](nil), columns...)}
}
...
//		Select(file.SetID).
//		Scan(ctx, &v)
func (_q *FileQuery) Select(selections ...ent.Selection) *FileSelect {
	return &FileSelect{query: _q, selections: append([]ent.Selection(nil), selections...)}
}

// Aggregate returns a FileSelect configured with the given aggregations.
func (_q *FileQuery) Aggregate(selections ...ent.Selection) *FileSelect {
...
func (_q *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if len(_q.withCounts) > 0 {
		...
		parents := make(map[int][]*File, len(nodes))
		...
		for _, edge := range _q.withCounts {
			...
			for rows.Next() {
				values, err := (*File)(nil).scanValues([]string{file.FieldID})
				if err != nil {
					rows.Close()
					return nil, err
				}
				...
				decoded := &File{}
				if err := decoded.assignValues([]string{file.FieldID}, values); err != nil {
					rows.Close()
					return nil, err
				}
...
func (_q *FileQuery) loadType(ctx context.Context, query *FileTypeQuery, nodes []*File, init func(*File), assign func(*File, *FileType)) error {
	...
	query.Where(filetype.ID.In(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
...
func (_q *FileQuery) loadField(ctx context.Context, query *FieldTypeQuery, nodes []*File, init func(*File), assign func(*File, *FieldType)) error {
	...
	if query.ctx.Limit != nil || query.ctx.Offset != nil {
		...
		query.modifiers = append(query.modifiers, func(selector *sql.Selector) {
			...
			partition := selector.C(file.FieldColumn)

			(&sqlgraph.NeighborsLimit{RowNumber: "ent_row_number", DefaultOrderField: fieldtype.FieldID, Offset: offset}).Modifier(partition, limit)(selector)
		})
	}
...
type FileGroupBy struct {
	query      *FileQuery
	columns    []ent.EntityColumn[entity.File]
	aggregates []ent.Selection
}

func (_g *FileGroupBy) Aggregate(selections ...ent.Selection) *FileGroupBy {
...
func (_g *FileGroupBy) Rows(ctx context.Context) ([]*ent.Row, error) {
	return _g.selectQuery().Rows(ctx)
}

func (_g *FileGroupBy) selectQuery() *FileSelect {
...
type FileSelect struct {
	...
	groups     []ent.EntityColumn[entity.File]
}

func (_s *FileSelect) Aggregate(selections ...ent.Selection) *FileSelect {
	_s.selections = append(_s.selections, selections...)
	return _s
}

func (_s *FileSelect) Row(ctx context.Context) (*ent.Row, error) {
...
func (_s *FileSelect) sqlQuery(ctx context.Context) (*sql.Selector, error) {
	root := _s.query.Clone()
	root.ctx.Fields = nil
	for _, selection := range _s.selections {
		if column := selection.Ref(); column.Name != "" && (column.Table == "" || column.Table == file.Table) {
...
func (_s *FileSelect) Scan(ctx context.Context, value any) error {
...
func (_s *FileSelect) Rows(ctx context.Context) ([]*ent.Row, error) {
	if len(_s.selections) == 0 {
