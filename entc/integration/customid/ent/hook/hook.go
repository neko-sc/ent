...
func (f BlobLinkFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.BlobLinkMutation); ok {
		return f(ctx, mv)
	}
...
func (f OtherFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.OtherMutation); ok {
		return f(ctx, mv)
	}
...
// The ValueScanFunc type is an adapter to allow the use of ordinary
// function as ValueScan mutator.
type ValueScanFunc func(context.Context, *ent.ValueScanMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ValueScanFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ValueScanMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ValueScanMutation", m)
}
