...
func (_u *PetUpdate) sqlSave(ctx context.Context) (_node int, err error) {
	...
	if _u.mutation.patch.Cars.Clear {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   pet.CarsTable,
	...
	if nodes := _u.mutation.patch.Cars.Remove; len(nodes) > 0 && !_u.mutation.patch.Cars.Clear {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   pet.CarsTable,
	...
	if nodes := _u.mutation.patch.carsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   pet.CarsTable,
