...
func (_q *UserQuery) Clone() *UserQuery {
	...
	if _q.withNamedInfo != nil {
		cloned.withNamedInfo = make(map[string]*InfoQuery, len(_q.withNamedInfo))
		for name, query := range _q.withNamedInfo {
			cloned.withNamedInfo[name] = query.Clone()
		}
	}

	if _q.withNamedRentals != nil {
		cloned.withNamedRentals = make(map[string]*RentalQuery, len(_q.withNamedRentals))
		for name, query := range _q.withNamedRentals {
			cloned.withNamedRentals[name] = query.Clone()
		}
	}
...
func (_q *UserQuery) loadMetadata(ctx context.Context, query *MetadataQuery, nodes []*User, init func(*User), assign func(*User, *Metadata)) error {
	...
	if query.ctx.Limit != nil || query.ctx.Offset != nil {
		...
		query.modifiers = append(query.modifiers, func(selector *sql.Selector) {
			...
			partition := selector.C(user.MetadataColumn)
...
func (_q *UserQuery) loadInfo(ctx context.Context, query *InfoQuery, nodes []*User, init func(*User), assign func(*User, *Info)) error {
	...
	if query.ctx.Limit != nil || query.ctx.Offset != nil {
		...
		query.modifiers = append(query.modifiers, func(selector *sql.Selector) {
			...
			partition := selector.C(user.InfoColumn)

			(&sqlgraph.NeighborsLimit{RowNumber: "ent_row_number", DefaultOrderField: info.FieldID, Offset: offset}).Modifier(partition, limit)(selector)
		})
	}
...
func (_q *UserQuery) loadRentals(ctx context.Context, query *RentalQuery, nodes []*User, init func(*User), assign func(*User, *Rental)) error {
	...
	if query.ctx.Limit != nil || query.ctx.Offset != nil {
		...
		query.modifiers = append(query.modifiers, func(selector *sql.Selector) {
			...
			partition := selector.C(user.RentalsColumn)

			(&sqlgraph.NeighborsLimit{RowNumber: "ent_row_number", DefaultOrderField: rental.FieldID, Offset: offset}).Modifier(partition, limit)(selector)
		})
	}
...
func (_q *UserQuery) WithNamedInfo(name string, opts ...func(*InfoQuery)) *UserQuery {
	...
	if _q.withNamedInfo == nil {
		_q.withNamedInfo = make(map[string]*InfoQuery)
	}
	_q.withNamedInfo[name] = query
	return _q
}
