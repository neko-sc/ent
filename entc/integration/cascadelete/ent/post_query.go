...
type PostQuery struct {
	config
	ctx        *QueryContext
	order      []ent.OrderOption[entity.Post]
...
func (_q *PostQuery) Where(predicates ...ent.Predicate[entity.Post]) *PostQuery {
	_q.predicates = append(_q.predicates, predicates...)
	return _q
}

func (_q *PostQuery) WhereP(predicates ...func(*sql.Selector)) *PostQuery {
...
func (_q *PostQuery) Join(table string, on ...func(*sql.Selector)) *PostQuery {
	return _q.JoinAs(table, "", on...)
}

func (_q *PostQuery) JoinAs(table, alias string, on ...func(*sql.Selector)) *PostQuery {
...
func (_q *PostQuery) LeftJoin(table string, on ...func(*sql.Selector)) *PostQuery {
	return _q.LeftJoinAs(table, "", on...)
}

func (_q *PostQuery) LeftJoinAs(table, alias string, on ...func(*sql.Selector)) *PostQuery {
	on = append([]func(*sql.Selector){}, on...)
	_q.joins = append(_q.joins, func(selector *sql.Selector) {
		joined := sql.Dialect(selector.Dialect()).Table(table)
		if alias != "" {
...
// Post, readable with Edges.Count. Only neighbors matching all predicates are
...
func (_q *PostQuery) WithCount[N, K any](edge ent.Relation[entity.Post, N, K], predicates ...ent.Predicate[N]) *PostQuery {
...
func (_q *PostQuery) Order(o ...ent.OrderOption[entity.Post]) *PostQuery {
...
// always included.
func (_q *PostQuery) Columns(columns ...ent.EntityColumn[entity.Post]) *PostQuery {
	if len(columns) == 0 {
...
func (_q *PostQuery) IDs(ctx context.Context) (ids []int, err error) {
	...
	if ids, err = ent.Values(ctx, _q.Select(post.ID), post.ID); err != nil {
		return nil, err
	}
	return ids, nil
}
...
func (_q *PostQuery) Clone() *PostQuery {
	...
	cloned := &PostQuery{
		config:     _q.config,
		ctx:        _q.ctx.Clone(),
		order:      append([]ent.OrderOption[entity.Post]{}, _q.order...),
		predicates: append([]ent.Predicate[entity.Post]{}, _q.predicates...),
...
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
func (_q *PostQuery) GroupBy(columns ...ent.EntityColumn[entity.Post]) *PostGroupBy {
	if len(columns) == 0 {
		panic("ent: GroupBy requires at least one column")
	}
	return &PostGroupBy{query: _q, columns: append([]ent.EntityColumn[entity.Post](nil), columns...)}
}
...
//		Select(post.Text).
//		Scan(ctx, &v)
func (_q *PostQuery) Select(selections ...ent.Selection) *PostSelect {
	return &PostSelect{query: _q, selections: append([]ent.Selection(nil), selections...)}
}

// Aggregate returns a PostSelect configured with the given aggregations.
func (_q *PostQuery) Aggregate(selections ...ent.Selection) *PostSelect {
...
func (_q *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if len(_q.withCounts) > 0 {
		...
		parents := make(map[int][]*Post, len(nodes))
		...
		for _, edge := range _q.withCounts {
			...
			for rows.Next() {
				values, err := (*Post)(nil).scanValues([]string{post.FieldID})
				if err != nil {
					rows.Close()
					return nil, err
				}
				...
				decoded := &Post{}
				if err := decoded.assignValues([]string{post.FieldID}, values); err != nil {
					rows.Close()
					return nil, err
				}
...
func (_q *PostQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*Post, init func(*Post), assign func(*Post, *Comment)) error {
	...
	if query.ctx.Limit != nil || query.ctx.Offset != nil {
		...
		query.modifiers = append(query.modifiers, func(selector *sql.Selector) {
			...
			partition := selector.C(post.CommentsColumn)

			(&sqlgraph.NeighborsLimit{RowNumber: "ent_row_number", DefaultOrderField: comment.FieldID, Offset: offset}).Modifier(partition, limit)(selector)
		})
	}
...
func (_q *PostQuery) ForUpdate(opts ...sql.LockOption) *PostQuery {
...
func (_q *PostQuery) ForShare(opts ...sql.LockOption) *PostQuery {
	if _q.driver.Dialect() == dialect.Postgres {
		_q.Unique(false)
	}
	_q.modifiers = append(_q.modifiers, func(s *sql.Selector) {
...
type PostGroupBy struct {
	query      *PostQuery
	columns    []ent.EntityColumn[entity.Post]
	aggregates []ent.Selection
}

func (_g *PostGroupBy) Aggregate(selections ...ent.Selection) *PostGroupBy {
...
func (_g *PostGroupBy) Rows(ctx context.Context) ([]*ent.Row, error) {
	return _g.selectQuery().Rows(ctx)
}

func (_g *PostGroupBy) selectQuery() *PostSelect {
...
type PostSelect struct {
	...
	groups     []ent.EntityColumn[entity.Post]
}

func (_s *PostSelect) Aggregate(selections ...ent.Selection) *PostSelect {
	_s.selections = append(_s.selections, selections...)
	return _s
}

func (_s *PostSelect) Row(ctx context.Context) (*ent.Row, error) {
...
func (_s *PostSelect) sqlQuery(ctx context.Context) (*sql.Selector, error) {
	root := _s.query.Clone()
	root.ctx.Fields = nil
	for _, selection := range _s.selections {
		if column := selection.Ref(); column.Name != "" && (column.Table == "" || column.Table == post.Table) {
...
func (_s *PostSelect) Scan(ctx context.Context, value any) error {
...
func (_s *PostSelect) Rows(ctx context.Context) ([]*ent.Row, error) {
	if len(_s.selections) == 0 {
