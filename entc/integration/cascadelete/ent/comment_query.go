...
func (_q *CommentQuery) Where(predicates ...ent.Predicate[entity.Comment]) *CommentQuery {
	_q.predicates = append(_q.predicates, predicates...)
	return _q
}

func (_q *CommentQuery) WhereP(predicates ...func(*sql.Selector)) *CommentQuery {
...
func (_q *CommentQuery) Join(table string, on ...func(*sql.Selector)) *CommentQuery {
	return _q.JoinAs(table, "", on...)
}

func (_q *CommentQuery) JoinAs(table, alias string, on ...func(*sql.Selector)) *CommentQuery {
...
func (_q *CommentQuery) LeftJoin(table string, on ...func(*sql.Selector)) *CommentQuery {
	return _q.LeftJoinAs(table, "", on...)
}

func (_q *CommentQuery) LeftJoinAs(table, alias string, on ...func(*sql.Selector)) *CommentQuery {
	on = append([]func(*sql.Selector){}, on...)
	_q.joins = append(_q.joins, func(selector *sql.Selector) {
		joined := sql.Dialect(selector.Dialect()).Table(table)
		if alias != "" {
...
func (_q *CommentQuery) Order(o ...ent.OrderOption[entity.Comment]) *CommentQuery {
...
// always included.
func (_q *CommentQuery) Columns(columns ...ent.EntityColumn[entity.Comment]) *CommentQuery {
	if len(columns) == 0 {
...
func (_q *CommentQuery) IDs(ctx context.Context) (ids []int, err error) {
	...
	if ids, err = ent.Values(ctx, _q.Select(comment.ID), comment.ID); err != nil {
		return nil, err
	}
	return ids, nil
}
...
//		GroupBy(comment.Text).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
func (_q *CommentQuery) GroupBy(columns ...ent.EntityColumn[entity.Comment]) *CommentGroupBy {
	if len(columns) == 0 {
		panic("ent: GroupBy requires at least one column")
	}
	return &CommentGroupBy{query: _q, columns: append([]ent.EntityColumn[entity.Comment](nil), columns...)}
}
...
//		Select(comment.Text).
//		Scan(ctx, &v)
func (_q *CommentQuery) Select(selections ...ent.Selection) *CommentSelect {
	return &CommentSelect{query: _q, selections: append([]ent.Selection(nil), selections...)}
}

// Aggregate returns a CommentSelect configured with the given aggregations.
func (_q *CommentQuery) Aggregate(selections ...ent.Selection) *CommentSelect {
...
func (_q *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if len(_q.withCounts) > 0 {
		...
		parents := make(map[int][]*Comment, len(nodes))
		...
		for _, edge := range _q.withCounts {
			...
			for rows.Next() {
				values, err := (*Comment)(nil).scanValues([]string{comment.FieldID})
				if err != nil {
					rows.Close()
					return nil, err
				}
				...
				decoded := &Comment{}
				if err := decoded.assignValues([]string{comment.FieldID}, values); err != nil {
					rows.Close()
					return nil, err
				}
...
type CommentGroupBy struct {
	query      *CommentQuery
	columns    []ent.EntityColumn[entity.Comment]
	aggregates []ent.Selection
}

func (_g *CommentGroupBy) Aggregate(selections ...ent.Selection) *CommentGroupBy {
...
func (_g *CommentGroupBy) Rows(ctx context.Context) ([]*ent.Row, error) {
	return _g.selectQuery().Rows(ctx)
}

func (_g *CommentGroupBy) selectQuery() *CommentSelect {
...
type CommentSelect struct {
	...
	groups     []ent.EntityColumn[entity.Comment]
}

func (_s *CommentSelect) Aggregate(selections ...ent.Selection) *CommentSelect {
	_s.selections = append(_s.selections, selections...)
	return _s
}

func (_s *CommentSelect) Row(ctx context.Context) (*ent.Row, error) {
...
func (_s *CommentSelect) sqlQuery(ctx context.Context) (*sql.Selector, error) {
	root := _s.query.Clone()
	root.ctx.Fields = nil
	for _, selection := range _s.selections {
		if column := selection.Ref(); column.Name != "" && (column.Table == "" || column.Table == comment.Table) {
...
func (_s *CommentSelect) Scan(ctx context.Context, value any) error {
...
func (_s *CommentSelect) Rows(ctx context.Context) ([]*ent.Row, error) {
	if len(_s.selections) == 0 {
